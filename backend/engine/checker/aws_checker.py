...
if __name__ == "__main__":
    ...
    rules    = load_policies()
    results  = run_checks(resources_input, rules)
    ...
    # Sort by severity then status (FAIL first)
    ...
    print(f"\n{'='*80}")
    print(f"  FINDINGS  ({len(findings)} checks evaluated)")
    print(f"{'='*80}")
    print(f"  {'STATUS':<7} {'SEV':<9} {'RULE':<16} {'RESOURCE':<30} {'REGION'}")
    print(f"  {'-'*7} {'-'*9} {'-'*16} {'-'*30} {'-'*12}")

    for f in sorted_findings:
        icon     = STATUS_ICONS.get(f["status"], f["status"])
        sev      = f["severity"] or ""
        rule_id  = (f["rule_id"] or "")[:16]
        resource = (f["resource_id"] or "")[:30]
        region   = (f["region"] or "")[:12]
        print(f"  {icon:<7} {sev:<9} {rule_id:<16} {resource:<30} {region}")

    # ── Print failures detail ──────────────────────────────────────────
    failures = [f for f in findings if f["status"] == "FAIL"]
    if failures:
        print(f"\n{'='*80}")
        print(f"  FAILED CHECKS DETAIL")
        print(f"{'='*80}")
        for f in failures:
            print(f"\n  Rule      : {f['rule_id']} — {f['rule_title']}")
            print(f"  Resource  : {f['resource_id']} ({f['region']})")
            print(f"  Severity  : {f['severity']}")
            print(f"  Actual    : {f['actual_value']}")
            print(f"  Expected  : operator={f['operator']} value={f['expected_value']}")
